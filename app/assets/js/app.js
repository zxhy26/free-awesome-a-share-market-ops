...
function applyLiveFlowSnapshot(snapshot, options = {}) {
  ...
  if (
    snapshot.clsWatch
    && String(snapshot.clsWatch.tradeDate || "") === coreTradeDate()
    && Array.isArray(snapshot.clsWatch.items)
  ) {
    state.data.indices.annotations = snapshot.clsWatch;
  }
...
function indexTurningAttributions(index) {
  return buildClsIndexAnnotationEvents(index, indexAnnotationFeed());
}

function updateIndexAttributionEvents() {
  for (const chart of state.charts || []) {
    chart.attributionEvents = indexTurningAttributions(chart.data);
    chart.annotationSource = "财联社盯盘公开板块事件";
  }
}
