...
async function fetchLocalMarketDetail(target) {
  if (!["http:", "https:"].includes(location.protocol)) return null;
  const url = new URL("/api/v1/market-detail", location.origin);
  const values = {
    code: target.code,
...
async function loadData() {
  ...
  const sources = [];
  let quote = null;
  const localResult = await fetchLocalMarketDetail(target);
  if (localResult) {
    state.klines = localResult.kline?.items || [];
    quote = localResult.quote || null;
    context.klineSource = localResult.kline?.source || "";
    sources.push(localResult.kline?.source, localResult.quote?.source);
    errors.push(...(localResult.errors || []));
  } else {
    const live = globalThis.AShareMobileLive;
    if (!live) {
      errors.push("公开行情模块未加载，当前仅展示发行包快照。");
    } else {
    const tasks = [
    ...
    if (klineState.status === "fulfilled") {
      state.klines = klineState.value.items || [];
      context.klineSource = klineState.value.source;
      sources.push(klineState.value.source);
    } else {
      state.klines = [];
