...
[assembly: AssemblyVersion("2.21.10.0")]
[assembly: AssemblyFileVersion("2.21.10.0")]
...
internal static class Program
{
    ...
    private const string RuntimeTag = "版本_20260810-2.21.10-基础版-跨平台";
    private const string MutexName = "Local\\AshareReviewLauncher_Basic_221100";
#elif SELF_EDITION
    ...
    private const string RuntimeTag = "版本_20260810-2.21.10-自用版-跨平台";
    private const string MutexName = "Local\\AshareReviewLauncher_Self_221100";
#elif CUSTOM_EDITION
    ...
    private const string RuntimeTag = "版本_20260810-2.21.10-定制版-跨平台";
    private const string MutexName = "Local\\AshareReviewLauncher_Custom_221100";
#else
    ...
    private const string MutexName = "Local\\AshareReviewLauncher_Member_221100";
#endif
    private const string LauncherVersion = "2.21.10";
#if BASIC_EDITION
    ...
    private static void RefreshBackgroundTasks(string runtimeRoot)
    {
        string scriptPath = Path.Combine(runtimeRoot, "程序", "应用", "backend", "安装盘中实时任务.ps1");
        ...
        {
            WriteBackgroundTaskLog(runtimeRoot, "未找到 Windows PowerShell，跳过后台任务重绑。");
            return;
        }
        ...
        try
        {
            ...
            using (Process process = Process.Start(startInfo))
            {
                if (process == null)
                {
                    WriteBackgroundTaskLog(runtimeRoot, "后台任务重绑进程未启动。");
                    return;
                }
                if (!process.WaitForExit(45000))
                {
                    try { process.Kill(); } catch { }
                    WriteBackgroundTaskLog(runtimeRoot, "后台任务重绑超时，软件继续启动。");
                    return;
                }
                if (process.ExitCode != 0)
                {
                    WriteBackgroundTaskLog(runtimeRoot, "后台任务重绑失败，退出码 " + process.ExitCode.ToString() + "。");
                    return;
                }
            }
            WriteBackgroundTaskLog(runtimeRoot, "后台任务已切换到当前版本运行目录。");
        }
        catch (Exception error)
        {
            WriteBackgroundTaskLog(runtimeRoot, "后台任务重绑异常：" + error.Message);
        }
    }

    private static void WriteBackgroundTaskLog(string runtimeRoot, string message)
    {
        try
        {
            string logPath = Path.Combine(runtimeRoot, ".background-task-refresh.log");
            File.AppendAllText(
                logPath,
